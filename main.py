...
def main():
    ...
    for chrm in popula:
        ...
        if chrlen < opt_len:
            opt_chr = chrm
            opt_len = chrlen
    print("Маємо точки(позначення міст) на карті - графік 1."
          "\nДалі обирається випадкова початкова точка і з неї шукається шлях по всіх точках, "
          "\nтобто рішення задачі комівояжера - графік 2")
    ...
    for cor in range(len(opt_chr) - 1):
        ...
        plt.plot(x, y, "b-")
    plt.title("Оптимальний шлях проходження міст\n")
    plt.plot(x_city, y_city, marker='o', lw='0', color='r')
    plt.show()
